...
void Playing_two::onEnter()
{
    Layer::onEnter();
    this->_eventDispatcher->addCustomEventListener("playerjointable",CC_CALLBACK_1(Playing_two::joinTable,this));

    this->_eventDispatcher->addCustomEventListener("playerleavetablebroadcast",CC_CALLBACK_1(Playing_two::leaveTableBroadcast,this));
    this->_eventDispatcher->addCustomEventListener("PlayingBroadcast_BuHua",CC_CALLBACK_1(Playing_two::buHuaBroadcast,this));
    this->_eventDispatcher->addCustomEventListener("playing_btn_chooseCard",CC_CALLBACK_1(Playing_two::selectCard,this));
    this->_eventDispatcher->addCustomEventListener("playerout",CC_CALLBACK_1(Playing_two::outCard,this));
    this->_eventDispatcher->addCustomEventListener("realoutcard",CC_CALLBACK_1(Playing_two::outcardformserver,this));
    this->_eventDispatcher->addCustomEventListener("getsiglecardbroadcast",CC_CALLBACK_1(Playing_two::systemgetsiglecardbroadcast,this));
    this->_eventDispatcher->addCustomEventListener("operateaskbroadcast",CC_CALLBACK_1(Playing_two::responseoperate,this));
    this->_eventDispatcher->addCustomEventListener("operatesuccess",CC_CALLBACK_1(Playing_two::resultofoperate,this));
    this->_eventDispatcher->addCustomEventListener("operatesuccessbroadcast",CC_CALLBACK_1(Playing_two::responseoperatebroadcast,this));
    this->_eventDispatcher->addCustomEventListener("responseminglouask",CC_CALLBACK_1(Playing_two::responseminglouask,this));
    this->_eventDispatcher->addCustomEventListener("responsemingloubroadcast",CC_CALLBACK_1(Playing_two::responsemingloubroadcast,this));
    this->_eventDispatcher->addCustomEventListener("responseoneroundendbroadcast",CC_CALLBACK_1(Playing_two::responseonroundendbroadcast,this));
    this->_eventDispatcher->addCustomEventListener("responsebreakbackbroadcast",CC_CALLBACK_1(Playing_two::responsebreakbackbroadcast,this));
    this->_eventDispatcher->addCustomEventListener("playerreconnect",CC_CALLBACK_1(Playing_two::responsereconnect,this));
    this->_eventDispatcher->addCustomEventListener("playing_broadcast_outCard",CC_CALLBACK_1(Playing_two::outCardBroadcast,this));
    this->_eventDispatcher->addCustomEventListener("responsedismissask",CC_CALLBACK_1(Playing_two::dismissAskBroadcast,this));
    this->_eventDispatcher->addCustomEventListener("responseplayeffect",CC_CALLBACK_1(Playing_two::responsepalyeffect,this));
	this->_eventDispatcher->addCustomEventListener("chat",CC_CALLBACK_1(Playing_two::responsechatbroadcast,this));
	this->_eventDispatcher->addCustomEventListener("customchat",CC_CALLBACK_1(Playing_two::responsecustomchatbroadcast,this));
	this->_eventDispatcher->addCustomEventListener("responsedismisssuccessbroadcast",CC_CALLBACK_1(Playing_two::dismisssuccessbroadcast,this));
	this->_eventDispatcher->addCustomEventListener("responsebreakbroadcast",CC_CALLBACK_1(Playing_two::responsebreakbroadcast,this));
	this->_eventDispatcher->addCustomEventListener("responsereadybroadcast",CC_CALLBACK_1(Playing_two::responsereadybroadcast,this));
	this->_eventDispatcher->addCustomEventListener("responseready",CC_CALLBACK_1(Playing_two::responseready,this));
	this->_eventDispatcher->addCustomEventListener("responsegetplayerinfo",CC_CALLBACK_1(Playing_two::responsegetplayerinfo,this));
	this->_eventDispatcher->addCustomEventListener("loseconnect",CC_CALLBACK_1(Playing_two::losconnect,this));
	this->_eventDispatcher->addCustomEventListener("responsegetresulttotal",CC_CALLBACK_1(Playing_two::addresulttotal,this));
	this->_eventDispatcher->addCustomEventListener("showresult3",CC_CALLBACK_1(Playing_two::showresult3,this));
	
    this->_eventDispatcher->addCustomEventListener("restartbyconfirm",CC_CALLBACK_1(Playing_two::restartbyLoseConfirm,this));

	this->_eventDispatcher->addCustomEventListener("responseresultpk",CC_CALLBACK_1(Playing_two::responsepkbroadcast,this));
	this->_eventDispatcher->addCustomEventListener("showresultpk",CC_CALLBACK_1(Playing_two::responseshowpk,this));

	this->_eventDispatcher->addCustomEventListener("dingqueaskbroadcast",CC_CALLBACK_1(Playing_two::responseaskdingquebroadcast,this));
	//this->_eventDispatcher->addCustomEventListener("showresultpk",CC_CALLBACK_1(Playing_two::responsedingqueanser,this));
	this->_eventDispatcher->addCustomEventListener("userdingquebroadcast",CC_CALLBACK_1(Playing_two::responseanserBroadcast,this));
	this->_eventDispatcher->addCustomEventListener("mydingquesuccess",CC_CALLBACK_1(Playing_two::mydingquesuccess,this));
...
void Playing_two::responseaskdingquebroadcast(EventCustom *evt)
{
	...
	for(int i=0;i<3;i++)
	{
	    ...
	    bt_ding->addClickEventListener(CC_CALLBACK_0(Playing_two::clickdingquecallback,this,i+1));
	}

}
void Playing_two::mydingquesuccess(EventCustom *evt)
{
	Node *n_action_dingque=nodeCards[0]->getChildByName("node_actions_dingque");
	for(int i=0;i<3;i++)
	{
...
void Playing_two::clickdingquecallback(int que_num)
{
	TableCommand::getInstance()->requestdingque(que_num);
}
void Playing_two::responseanserBroadcast(EventCustom *evt)
{
...
void Playing_two::restartbyLoseConfirm(EventCustom *evt)
{
	restarapp_main(0);
}
void Playing_two::responseshowpk(EventCustom *evt)
{
...
void Playing_two::responsebreakbroadcast(EventCustom *evt)
{
...
void Playing_two::responsepkbroadcast(EventCustom *evt)
{
...
void Playing_two::restarapp_main(float dt)
{
...
void Playing_two::losconnect(EventCustom *evt)
{

	tp_lose->setVisible(true);
	tp_lose->setZOrder(10000);
}
void Playing_two::responsereconnect(EventCustom *evt)
{
    m_mjList.clear();
...
//	refreshoutcardByViewId(2);
    ...
    for (int i = 0; i < 2; i++)
    {
		...
		{
			nodePlayer->setPosition(posWait_two[i]);
			if(g_m->getStatus()==5)
			{
		...
		{
			nodePlayer->setPosition(posGame_two[i]);
			if(g_m->getStatus()==3)
			{
...
void Playing_two::playluxiang_player(std::vector<playluxiang> arr_luxiang)
{
...
void Playing_two::refreshminglou(int viewid)
{
   
}
void Playing_two::mingloufromluxiang(playluxiang p)
{
   

}
void Playing_two::dispathercardsfromluxing(playluxiang p)
{
...
void Playing_two::refreshjipaifromluxiang(int viewid)
{
...
void Playing_two::chupaifomluxiang(playluxiang p)
{
        ...
        if(arr_shoupai[viewid].at(i)==p.cardid[0])
        {
			...
			if(p.cardid[0]==17)//这是张 鸡牌
			{
				if(right_ji_num_luxiang==0)//这是 冲锋鸡
				{
					...
					CallFunc *visible=CallFunc::create(CC_CALLBACK_0(Playing_two::sp_visible,this,hua,true));
					DelayTime *de=DelayTime::create(0.5*luxiang_rate);
					CallFunc *invisible=CallFunc::create(CC_CALLBACK_0(Playing_two::sp_visible,this,hua,false));
					CallFunc *nextstep=CallFunc::create(CC_CALLBACK_0(Playing_two::updateactions,this,0));
	...
	if(p.cardid[0]!=17)
	{
		...
		CallFunc *isvisible=CallFunc::create(CC_CALLBACK_0(Playing_two::sp_visible,this,sp_mj_out,true));
		CallFunc *invisible=CallFunc::create(CC_CALLBACK_0(Playing_two::sp_visible,this,sp_mj_out,false));
		...
		CallFunc *refreshout=CallFunc::create(CC_CALLBACK_0(Playing_two::refreshchupaifromluxiang,this,viewid));
		CallFunc *nextstep=CallFunc::create(CC_CALLBACK_0(Playing_two::updateactions,this,0));
...
void Playing_two::refreshchupaifromluxiang(int viewid)
{
...
void Playing_two::refreshshoupaifromluxiang(int viewid)
{
...
int Playing_two::getlogicvalue(int a)
{
    return a/16*10+a%16;
}
void Playing_two::mopaifromluxiang(playluxiang p)
{
    int card=p.cardid[0];
...
void Playing_two::buhuafromluxiang(playluxiang p)
{
    int cardlen=p.cardlen;
    int uerid=p.actionuerid;
    int viewid=GetViewID(GamerData::getInstance()->getSeatId(),GamerDataDeal::getInstance()->getGamerData(uerid)->getSeatId());
...
void Playing_two::leftchifromluxiang(playluxiang p)
{
...
void Playing_two::refreshxingpaifromluxiang(int viewid)
{
...
void Playing_two::minggangfromluxiang(playluxiang p)
{
    
}
void Playing_two::angangfromluxiang(playluxiang p)
{
    
}
void Playing_two::refreshzhishifromluxiang(int viewid)
{
	GameController * controlLayer22 = (GameController *)this->getChildByTag(119);
	controlLayer22->setArrowDirection(viewid);
}
void Playing_two::nextstep(float dt)
{
...
void Playing_two::updateactions(float dt)
{
    schedule(schedule_selector(Playing_two::nextstep),2*luxiang_rate);
}
void Playing_two::hufromluxiang(playluxiang p)
{
    int uerid=p.actionuerid;
    int provide=p.provideuserid;
...
void Playing_two::initplayer()
{
    ...
    for(int i=0;i<2;i++)
    {
        ...
        Node *n_player=nodePlayers[i];
		
        n_player->setPosition(posGameluxiang_two[i]);
		
		nodePlayers[i]->getParent()->setZOrder(100);
    ...
    //加载录像行牌界面
    Node *n_luxiang=CSLoader::createNode("res/loading/Layer_luxiang_two.csb");
	...
	bt_back->addClickEventListener(CC_CALLBACK_0(Playing_two::removeluxiang,this));

	Button *bt_next=(Button *)sp_bg->getChildByName("Button_next");
	bt_next->addClickEventListener(CC_CALLBACK_0(Playing_two::nextfromluxiang,this));
	Button *bt_before=(Button *)sp_bg->getChildByName("Button_befor");
	bt_before->addClickEventListener(CC_CALLBACK_0(Playing_two::beforefromluxiang,this));
	bt_pause_luxiang=(Button *)sp_bg->getChildByName("Button_pause");
	bt_pause_luxiang->addClickEventListener(CC_CALLBACK_0(Playing_two::pausefromluxiang,this));
...
void Playing_two::nextfromluxiang()
{
	if(luxiang_rate>0.1)
	{
...
void Playing_two::beforefromluxiang()
{
	if(luxiang_rate<=3)
	{
		luxiang_rate=luxiang_rate+0.1;
...
void Playing_two::pausefromluxiang()
{
	if(bt_pause_luxiang->getTag()==0)
	{
...
void Playing_two::responsepalyeffect(EventCustom *evt)
{
...
void Playing_two::removeluxiang()
{
    this->removeFromParent();

}
void Playing_two::chushihua(bool isluxiang)
{
...
bool Playing_two::init()
{
	...
	tp_lose->setVisible(false);
	 ...
	 touchListener->onTouchBegan = CC_CALLBACK_2(Playing_two::onTouchBegan,this);//触摸开始
	 touchListener->onTouchMoved = CC_CALLBACK_2(Playing_two::onTouchMoved,this);//触摸移动
	 touchListener->onTouchEnded = CC_CALLBACK_2(Playing_two::onTouchEnded,this);//触摸结束
	 _eventDispatcher->addEventListenerWithSceneGraphPriority(touchListener,this);//注册分发器

    return true;
}

bool Playing_two::onTouchBegan(Touch* touch, Event *event)
{
	//遍历手里的牌看看点到了那张
...
void Playing_two::onTouchMoved(Touch* touch, Event *event)
{
...
void Playing_two::onTouchEnded(Touch* touch, Event *event)
{
		...
		if(m_mjList.size()>outcardingId)
		{
			if(m_mjList.at(outcardingId)->getIsCardSelected()==true)//牌已经被提起了就直接打出去
			{
				...
				if(UserDefault::getInstance()->getBoolForKey("iscanoperatecard")==true&&m_mjList.at(outcardingId)->iscanoperateornot==true)
				{
					...
					iscanchupai=false;
					// scheduleOnce(schedule_selector(Playing_two::recovercupai),1);
					for(int i=0;i<m_mjList.size();i++)
					{
...
void Playing_two::recovercupai(float dt)
{
	iscanchupai=true;
}
//加载所有玩家显示初始化(玩家等待）
void Playing_two::loadAndShowPlayers()
{
	...
	Node *n_all=CSLoader::createNode("res/loading/mj_two.csb");
	addChild(n_all);
	Sprite *sp_icon_three=(Sprite *)n_all->getChildByName("text_wanfa_two_1");
	Sprite *sp_icon_three_two=(Sprite *)n_all->getChildByName("text_wanfa_twotwo_3");
	if(MjData::getInstance()->getpalyrule()==3)
	{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        mj_nodePLayer->setVisible(false);
			...
			mj_nodePLayer->setPosition(posWait_two[i]);
			Button *bt_head=(Button *)mj_nodePLayer->getChildByName("Button_headclick");
			bt_head->setTag(i+1);
			bt_head->addTouchEventListener(CC_CALLBACK_2(Playing_two::ButtonHandler,this));
...
void Playing_two::refreshdingque(int quedemen,int viewid)
{
...
void Playing_two::setcards_detail_actionsisvisible(bool isvisible)
{
    node_detail_actions->setVisible(isvisible);
}
void Playing_two::refreshactionsbutton(bool iscanoperate)
{
...
//按钮回调函数
void Playing_two::ButtonHandler(Ref* pRef, ui::Widget::TouchEventType touchType)
{
...
void Playing_two::responsegetplayerinfo(EventCustom *evt)
{
...
//游戏开始(玩家头像位置更新等)
void Playing_two::startGame_gamePlayer(int banker, int left_card_cnt)
{
    isminglou_ing=false;
    isminglou_over=false;
    ...
    for (int i = 0; i <2; i++)
    {
        ...
        nodePlayer->runAction(MoveTo::create(0.5f,posGame_two[i]));
...
void Playing_two::getheadpathlocalbyurl(std::string url,int tag)
{
    cocos2d::network::HttpRequest* request = new cocos2d::network::HttpRequest();
    request->setUrl(url.c_str());
    request->setRequestType(cocos2d::network::HttpRequest::Type::GET);
    request->setResponseCallback(this, httpresponse_selector(Playing_two::onGetFinished));
...
void Playing_two::onGetFinished(cocos2d::network::HttpClient *sender, cocos2d::network::HttpResponse *response)
{
    if (!response)
    {
        return;
...
//玩家加入桌子fset
void Playing_two::joinTable(EventCustom* evt)
{
    ...
    {//当前玩家
        ...
        btnUserInfo->addTouchEventListener(CC_CALLBACK_2(Playing_two::ButtonHandler, this));
...
//玩家离开桌子广播
void Playing_two::leaveTableBroadcast(EventCustom* evt)
{
...
void Playing_two::responsebreakbackbroadcast(EventCustom *evt)
{
...
//玩家补花广播
void Playing_two::buHuaBroadcast(EventCustom* evt)
{
	/*
...
//发牌
void Playing_two::dispacherCard(google::protobuf::RepeatedField< ::google::protobuf::int32 > cards)
{
...
//当前玩家选中要出的牌(本地)
void Playing_two::selectCard(EventCustom* evt)
{
    if (mj_selectedMj!=nullptr)
    {
...
void Playing_two::selectCardbylocal(MjNode *mj)
{
...
//出牌(本地)
void Playing_two::outCardBroadcast(EventCustom* evt)
{
    TableCommand::getInstance()->requestOutCard(mj_selectedMj->id);
}
void Playing_two::spsetvisible(Sprite *sp,bool isvisible)
{
	sp->setVisible(isvisible);
}
void Playing_two::outcardformserver(EventCustom *evt)
{
	refreshactionsbutton(false);
	...
	//	m_current_diamond->setPosition(Vec2(card_sp->getContentSize().width/2,y_diamond));
		...
		CallFunc *call=CallFunc::create(CC_CALLBACK_0(Playing_two::refreshoutcardByViewId,this,viewId));
		CallFunc *call_visible=CallFunc::create(CC_CALLBACK_0(Playing_two::sp_visible,this,sp_poker_out,true));
		CallFunc *call_unvisible=CallFunc::create(CC_CALLBACK_0(Playing_two::sp_visible,this,sp_poker_out,false));
		MoveTo *m_t=MoveTo::create(0,origin_position);
		Sequence *se1=Sequence::create(call_unvisible,m_t,nullptr);
		Sequence *se=Sequence::create(call_visible,action1,de1,action,se1,call,RotateTo::create(0,0),CallFunc::create(CC_CALLBACK_0(Playing_two::playeeffect_normal,this,"audio_card.mp3")),nullptr);
        sp_poker_out->runAction(se);
        scheduleOnce(schedule_selector(Playing_two::delayShowCurrentDiamond),0.8);
        hands_cnt[viewId]=hands_cnt[viewId]-1;
 ...
 void Playing_two::delayShowCurrentDiamond(float dt)
 {
	 m_current_diamond->setVisible(true);
 }
void Playing_two::minglououtcarddelay(float dt)
{
	SEND_CUSTOM_MSG("playing_broadcast_outCard");
}
void Playing_two::systemgetsiglecardbroadcast(EventCustom *evt)
{
        ...
        if(userid==GamerData::getInstance()->getUId())//当前玩家是自己
        {
			...
			if(isminglou_over)
			{
                if(!ishu)//这张牌不能胡
                {
					...
					scheduleOnce(schedule_selector(Playing_two::minglououtcarddelay),0.5);
                }
            }
        }
        else
...
void Playing_two::responseoperate(EventCustom *evt)
{
    arr_operates.clear();
    ...
    for(int i=0;i<actions.size();i++)
    {
        ...
        if(actions.Get(i)!=0)
        {
             if(actions.Get(i)==operate_showpeng)//碰牌
            {
                ...
                arr_actions[button_num-1]->addClickEventListener(CC_CALLBACK_0(Playing_two::peng_callfunc,this,operate_showpeng,actioncard));
            }
            else if(actions.Get(i)==operate_showgang)//杠牌
            {
                ...
                arr_actions[button_num-1]->addClickEventListener(CC_CALLBACK_0(Playing_two::gang_callfunc,this,arr_gang));
            ...
            else if(actions.Get(i)==operate_showting)//听牌
            {
                ...
                arr_actions[button_num-1]->addClickEventListener(CC_CALLBACK_0(Playing_two::minglou_callfunc,this));//此处进行听牌操作
            }
            else if(actions.Get(i)==operate_showhu)//胡牌
            {
                ...
                arr_actions[button_num-1]->addClickEventListener(CC_CALLBACK_0(Playing_two::hu_callfunc,this,actioncard));
            }
        }
    }
    ...
    arr_actions[button_num]->addClickEventListener(CC_CALLBACK_0(Playing_two::guo_callfunc,this));
...
//操作结果
void Playing_two::resultofoperate(EventCustom *evt)
{
...
void Playing_two::hupai_yinxiao(int op,int sex_)
{
	Tools::playOperateByGender(op,sex_);
}
void Playing_two::playeeffect_normal(std::string path)
{
	Tools::playEffect(path.c_str());
}
//操作结果广播
void Playing_two::responseoperatebroadcast(EventCustom *evt)
{
       ...
       // Tools::playOperateByGender(operatecode, p->getSex());
		...
		else  if(operatecode==8)//胡
		{
			...
			if(uid==provideuid) 
			{
				...
				CallFunc *call=CallFunc::create(CC_CALLBACK_0(Playing_two::hupai_yinxiao,this,10,p->getSex()));
				Sequence *se=Sequence::create(de,call,CallFunc::create(CC_CALLBACK_0(Playing_two::removeSp,this,sp_)),nullptr);
				sp_->runAction(se);
			}
			else
			{
...
void Playing_two::playAction_hu_animation()
{
	...
	for(int i=0;i<arr_hutype.size();i++)
	{
		 ...
		 if(hu_t.gangtype!=0)
		 {
			 ...
			 //放杠的动画 然后再放胡的动画 
             CallFunc *call_gang=CallFunc::create(CC_CALLBACK_0(Playing_two::playAction_gang_animationSigle,this,hu_t.viewid,hu_t.gangtype));
			 DelayTime *de_gang=DelayTime::create(1);
			 CallFunc *call_hu=CallFunc::create(CC_CALLBACK_0(Playing_two::playAction_hu_animationSigile,this,hu_t.viewid,hu_t.hutype));
			 DelayTime *de_hu=DelayTime::create(1);
			 CallFunc *call_remove=CallFunc::create(CC_CALLBACK_0(Playing_two::removeSp,this,sp_temp_));
	...
	if(arr_hutype.size()>1)
	{
		...
		CallFunc *call_yipao=CallFunc::create(CC_CALLBACK_0(Playing_two::playAction_YipaoDuoxiangAnimation,this,arr_hu_viewid,arr_hutype.size()));
...
void Playing_two::removeSp(Sprite *sp)
{
	sp->removeFromParent();
}
void Playing_two::showRresultLayerByOverAnimation(float dt)
{
	cleanallscene();
...
void Playing_two::playAction_YipaoDuoxiangAnimation( std::vector<int >arr_hu_viewids,int hu_num)
{
	
}
void Playing_two::playAction_gang_animationSigle(int viewid,int gang_type)
{
	...
	Sequence *se=Sequence::create(CallFuncN::create(CC_CALLBACK_0(Playing_two::sp_visible,this,sp_action_animation,true)),DelayTime::create(0.5),CallFuncN::create(CC_CALLBACK_0(Playing_two::sp_visible,this,sp_action_animation,false)),nullptr);
	sp_action_animation->runAction(se);
}
void Playing_two::playAction_hu_animationSigile(int viewid,int hu_type)
{
	Sprite *sp_action_animation=(Sprite *)nodePlayers[viewid]->getChildByName("icon_action_animation");
	sp_action_animation->setVisible(true);
	if(hu_type==1)
...
void Playing_two::playeraction_animation(int viewid,int operatecode)
{
    Sprite *sp_action_animation=(Sprite *)nodePlayers[viewid]->getChildByName("icon_action_animation");
    ...
    Sequence *se=Sequence::create(CallFuncN::create(CC_CALLBACK_0(Playing_two::sp_visible,this,sp_action_animation,true)),DelayTime::create(0.2),st1,st,CallFuncN::create(CC_CALLBACK_0(Playing_two::spsettexture,this,sp_action_animation,texturepath)),DelayTime::create(0.8),CallFuncN::create(CC_CALLBACK_0(Playing_two::sp_visible,this,sp_action_animation,false)),ScaleTo::create(0,1.0),nullptr);
    sp_action_animation->runAction(se);
}
void Playing_two::spsettexture(Sprite *sp,std::string path)
{
	sp->setTexture(path.c_str());
}
void Playing_two::playAction_jiAnimation(int viewid,int jitype)
{
	Sprite *sp_action_animation=(Sprite *)nodePlayers[viewid]->getChildByName("icon_action_animation");
...
void Playing_two::sp_visible(Sprite *sp,bool isvisible)
{
    sp->setVisible(isvisible);
}
void Playing_two::refreshjipaiOfPlayer(int viewid)
{
	Node *node_jipai=nodeCards[viewid]->getChildByName("node_ji");
...
void Playing_two::refreshoutcardByViewId(int viewid)
{
	//刷新玩家打出的牌
...
void Playing_two::refreshxingpaiposofothers(std::vector<operate_result>arr_mjs,Node *parent,int viewid ,int ismopai)
{
	//手牌重新整理
	if(ismopai==0)
	{
	...
	else
	{
		for(int i=14;i>0;i--)
		{
			...
			if(i==1)
			{
				if(viewid==1)
				{
					i_m->setPosition(i_m->getPosition()+Vec2(-30,0));
				}
				else if(viewid==2)
				{
					i_m->setPosition(i_m->getPosition()+Vec2(-30,0));
...
void Playing_two::sortpokersbywhx(std::vector<MjNode *>arr_mjs)
{
...
void Playing_two::sortMjByLaiya(std::vector<int > &arr_mjs)
{
	std::sort(arr_mjs.begin(), arr_mjs.end(), [](int a,int b){
		return a<b;
	});
}
void Playing_two::sortMjByLaiya_cardshow(std::vector<int> &arr_poker)
{
	std::sort(arr_poker.begin(), arr_poker.end(), [](int a,int b){
		return a>b;
	});
}
void Playing_two::refreshxingpaipos(std::vector<operate_result>arr_poker, Node *parent)
{
	int pos_origin_x=0;
...
//碰牌回调
void Playing_two::peng_callfunc(int opreratecode,int action_card)
{
    TableCommand::getInstance()->requestoperate(opreratecode,action_card);
}
//杠牌回调
void Playing_two::gang_callfunc(std::vector<operate_me>gang_arr)
{
    if(gang_arr.size()!=0)
    {
        if(gang_arr.size()==1)
...
//胡牌回调
void Playing_two::hu_callfunc(int actioncard)
{
    TableCommand::getInstance()->requestoperate(7,actioncard);
}
//明楼回调
void Playing_two::minglou_callfunc()
{
    //点击明楼 要发起请求
    TableCommand::getInstance()->requestminglouask();
}
void Playing_two::responseready(EventCustom *evt)
{
...
void Playing_two::responsereadybroadcast(EventCustom *evt)
{
...
void Playing_two::responseminglouask(EventCustom *evt)
{
    refreshactionsbutton(false);
...
void Playing_two::responsemingloubroadcast(EventCustom *evt)
{
...
void Playing_two::responseonroundendbroadcast(EventCustom *evt)
{
	//数据
	...
	if(arr_hutype.size()!=0)
	{
		...
		scheduleOnce(schedule_selector(Playing_two::showfanpaiji),time_all);
		time_all=time_all+3;
	}
	
	//展示结果界面
	scheduleOnce(schedule_selector(Playing_two::showRresultLayerByOverAnimation),time_all);

}
void Playing_two::refreshcard_show(oneroundresult r_show)
{
...
void Playing_two::showfanpaiji(float dt)
{
...
void Playing_two::dismisssuccessbroadcast(EventCustom *evt)
{
	diamisssuccess=2;
}
void Playing_two::dismissAskBroadcast(EventCustom *evt)
{
    //数据
...
//显示可以吃的牌
void Playing_two::showcards_chi(std::vector<operate_me > chi_arr)
{
    refreshactionsbutton(false);
    ...
    for(int i=0;i<chi_arr.size();i++)
    {
        ...
        bt_call_chi->addClickEventListener(CC_CALLBACK_0(Playing_two::chi_final_request,this,chi_arr[i].operatecode,arr_cars));
    }
}
void Playing_two::chi_final_request(int opcode,std::vector<int >arr_cards)
{
    TableCommand::getInstance()->requestoperate(opcode,arr_cards[0]);
    setcards_detail_actionsisvisible(false);
}
//显示可以杠的牌
void Playing_two::showcards_gang(std::vector<operate_me>gang_arr)
{
    setcards_detail_actionsisvisible(true);
    ...
    for(int i=0;i<gang_arr.size();i++)
    {
        ...
        arr_cars.push_back(mj_real_value);
        arr_cars.push_back(mj_real_value);
        bt_call_chi->addClickEventListener(CC_CALLBACK_0(Playing_two::chi_final_request,this,gang_arr[i].operatecode,arr_cars));
    }
}
//直接吃牌
void Playing_two::operate_chi(operate_me op)
{
...
//杠牌
void Playing_two::oprate_gang(operate_me op)
{
    int opcode=op.operatecode;
    int cardid=op.card_id;
...
//当前玩家出牌返回（服务器）
void Playing_two::outCard(EventCustom* evt)
{
...
//刷新麻将位置
void Playing_two::refreshMjsPos()
{
...
///排列pMjNode 从大到小
void Playing_two::sortMjs(Vector<MjNode*> &_pokerVec)
{
...
//补花动画显示并且修改其数量
void Playing_two::buhuaActionManage(int uid,int numHua)
{
    auto action = Sequence::create(DelayTime::create(1.0f),FadeOut::create(0.25f),CallFuncN::create( CC_CALLBACK_1(Playing_two::buHuaActionEndcallback, this)),nullptr);
...
//补花提示icon显示结束
void Playing_two::buHuaActionEndcallback(Node* sender )
{
    sender->setVisible(false);
    sender->setOpacity(255);
}
//出牌本地动画结束后的回调函数
void Playing_two::outCard1Action(Node* sender )
{
    MjNode* card = (MjNode*)sender;
    TableCommand::getInstance()->requestOutCard(card->id);
}
void Playing_two::setcardoperateornot(bool is)
{
    for(int i=0;i<m_mjList.size();i++)
    {
...
void Playing_two::cleanallscene()
{
...
void Playing_two::playEffectCustom(std::string kName,int viewid)
{
    ...
    log("%s",kName.c_str());
    experimental::AudioEngine::setFinishCallback(custom_id,CC_CALLBACK_0(Playing_two::on_effect_over,this,viewid));

}

void Playing_two::on_effect_over(int viewid)
{
	if(viewid==0)
	{
		UserDefault::getInstance()->setBoolForKey("isplayluyin",false) ;
...
void Playing_two::responsechatbroadcast(EventCustom *evt)
{
	...
	if(UserDefault::getInstance()->getBoolForKey("isplayluyin")==false)
	{
		if(index>100&&index<200)
		{
			...
			sp->setVisible(false);
			addChild(sp);
			Sequence *se=Sequence::create(DelayTime::create(2.0f),CallFunc::create(CC_CALLBACK_0(Playing_two::recoverchat,this,viewid,sp)),nullptr);
		    sp->runAction(se);
			GamerData *g=GamerDataDeal::getInstance()->getGamerData(uid);
	...
	if(index>200)
	{
		...
		RepeatForever *re=RepeatForever::create(an);
		emoji->runAction(re);
		Sequence *se=Sequence::create(DelayTime::create(2.0f),CallFunc::create(CC_CALLBACK_0(Playing_two::sp_visible,this,emoji,false)),nullptr);
		emoji->runAction(se);
	}
}
void Playing_two::recoverchat(int viewid,Sprite *sp)
{
	UserDefault::getInstance()->setBoolForKey("isplayluyin",false); 
...
void Playing_two::responsecustomchatbroadcast(EventCustom *evt)
{
...
void Playing_two::initcardWithCardIds(Sprite *sp_card,int cardid)
{
